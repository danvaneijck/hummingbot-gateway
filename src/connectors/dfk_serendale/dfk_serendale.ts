...
import { DfkSerendaleConfig } from './dfk_serendale.config';
import routerAbi from './defikingdoms_router.json';
import { ContractInterface } from '@ethersproject/contracts';
import { isFractionString } from '../../services/validators';

import {
  Router as klaytnRouter,
  Pair as klaytnPair,
  SwapParameters,
...
export class DfkSerendale implements Uniswapish {
  private static _instances: { [name: string]: DfkSerendale };
  private chain: Klaytn | DfkChain;
  ...
  private constructor(chain: string, network: string) {
    const config = DfkSerendaleConfig.config;

    this.chain = Klaytn.getInstance(network);
    this._factory = '0x36fAE766e51f17F8218C735f58426E293498Db2B';
    this._initCodeHash =
  ...
  public static getInstance(chain: string, network: string): DfkSerendale {
    if (DfkSerendale._instances === undefined) {
      DfkSerendale._instances = {};
    }
    if (!(chain + network in DfkSerendale._instances)) {
      DfkSerendale._instances[chain + network] = new DfkSerendale(
        chain,
        network
      );
    }

    return DfkSerendale._instances[chain + network];
  }
  ...
  public getAllowedSlippage(allowedSlippageStr?: string): Percent {
    ...
    const allowedSlippage = DfkSerendaleConfig.config.allowedSlippage;
  ...
  async estimateSellTrade(
    ...
    const pair: klaytnPair = await this.fetchPairData(quoteToken, baseToken);
    const trades: klaytnTrade<Token, Token, TradeType.EXACT_INPUT>[] =
      klaytnTrade.bestTradeExactIn([pair], baseTokenAmount, quoteToken, {
        maxHops: 1,
      });
  ...
  async estimateBuyTrade(
    ...
    const pair: klaytnPair = await this.fetchPairData(quoteToken, baseToken);
    const trades: klaytnTrade<Token, Token, TradeType.EXACT_OUTPUT>[] =
      klaytnTrade.bestTradeExactOut([pair], quoteToken, baseTokenAmount, {
        maxHops: 1,
      });
    if (!trades || trades.length === 0) {
      throw new UniswapishPriceError(
  ...
  async executeTrade(
    wallet: Wallet,
    trade: klaytnTrade<Token, Token, TradeType>,
    gasPrice: number,
    dfkRouter: string,
    ttl: number,
    ...
    const result: SwapParameters = klaytnRouter.swapCallParameters(trade, {
      ttl,
  ...
  async fetchPairData(tokenA: Token, tokenB: Token): Promise<klaytnPair> {
    return await klaytnFetcher.fetchPairData(
      tokenA,
      tokenB,
      this._factory,
      this._initCodeHash
