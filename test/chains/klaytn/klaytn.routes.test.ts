...
const patchGetNonce = () => {
  patch(klaytn.nonceManager, 'getNonce', () => 2);
};

const patchGetTokenBySymbol = () => {
  patch(klaytn, 'getTokenBySymbol', () => {
    return {
      chainId: 43114,
...
const patchGetERC20Allowance = () => {
  patch(klaytn, 'getERC20Allowance', () => ({ value: 1, decimals: 3 }));
};

const patchGetNativeBalance = () => {
  patch(klaytn, 'getNativeBalance', () => ({ value: 1, decimals: 3 }));
};

const patchGetERC20Balance = () => {
  patch(klaytn, 'getERC20Balance', () => ({ value: 1, decimals: 3 }));
};
...
describe('POST /evm/approve', () => {
  it('should return 200', async () => {
    patchGetWallet();
    klaytn.getContract = jest.fn().mockReturnValue({
      address,
    });
    patch(klaytn.nonceManager, 'getNonce', () => 115);
...
describe('POST /evm/allowances', () => {
  it('should return 200 asking for allowances', async () => {
    ...
    klaytn.getSpender = jest.fn().mockReturnValue(spender);
    klaytn.getContract = jest.fn().mockReturnValue({
    ...
    await request(gatewayApp)
      ...
      .expect((res) => expect(res.body.approvals.oUSDT).toEqual('0.001'));
  });
});
...
describe('POST /network/balances', () => {
  it('should return 200 asking for supported tokens', async () => {
    ...
    await request(gatewayApp)
      ...
      .expect((res) => expect(res.body.balances.oUSDT).toBeDefined());
  });
});
...
describe('POST /evm/cancel', () => {
  it('should return 200', async () => {
    ...
    klaytn.getWallet = jest.fn().mockReturnValue({
      address,
    });

    klaytn.cancelTx = jest.fn().mockReturnValue({
...
describe('POST /network/poll', () => {
  it('should get a NETWORK_ERROR_CODE when the network is unavailable', async () => {
    patch(klaytn, 'getCurrentBlockNumber', () => {
  ...
  it('should get a null in txReceipt for Tx in the mempool', async () => {
    patch(klaytn, 'getCurrentBlockNumber', () => 1);
    patch(klaytn, 'getTransaction', () => transactionOutOfGas);
    patch(klaytn, 'getTransactionReceipt', () => null);
    const res = await request(gatewayApp).post('/network/poll').send({
      chain: 'klaytn',
      network: 'mainnet',
      txHash:
  ...
  it('should get a null in txReceipt and txData for Tx that didnt reach the mempool and TxReceipt is null', async () => {
    patch(klaytn, 'getCurrentBlockNumber', () => 1);
    patch(klaytn, 'getTransaction', () => null);
    patch(klaytn, 'getTransactionReceipt', () => null);
    const res = await request(gatewayApp).post('/network/poll').send({
      chain: 'klaytn',
      network: 'mainnet',
  ...
  it('should get txStatus = 1 for a succesful query', async () => {
    patch(klaytn, 'getCurrentBlockNumber', () => 1);
    patch(klaytn, 'getTransaction', () => transactionSuccesful);
    patch(klaytn, 'getTransactionReceipt', () => transactionSuccesfulReceipt);
    const res = await request(gatewayApp).post('/network/poll').send({
      chain: 'klaytn',
      network: 'mainnet',
      txHash:
