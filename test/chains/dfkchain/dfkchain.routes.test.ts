...
const patchGetNonce = () => {
  patch(dfkchain.nonceManager, 'getNonce', () => 2);
};
...
const patchGetERC20Allowance = () => {
  patch(dfkchain, 'getERC20Allowance', () => ({ value: 1, decimals: 3 }));
};

const patchGetNativeBalance = () => {
  patch(dfkchain, 'getNativeBalance', () => ({ value: 1, decimals: 3 }));
};

const patchGetERC20Balance = () => {
  patch(dfkchain, 'getERC20Balance', () => ({ value: 1, decimals: 3 }));
};
...
describe('POST /evm/approve', () => {
  it('should return 200', async () => {
    ...
    patch(dfkchain.nonceManager, 'getNonce', () => 115);
...
describe('POST /evm/allowances', () => {
  it('should return 200 asking for allowances', async () => {
    ...
    await request(gatewayApp)
      ...
      .expect((res) => expect(res.body.approvals.xJEWEL).toEqual('0.001'))
      .expect((res) => expect(res.body.approvals.USDC).toEqual('0.001'));
  });
});
...
describe('POST /network/balances', () => {
  it('should return 200 asking for supported tokens', async () => {
    ...
    await request(gatewayApp)
      ...
      .expect('Content-Type', /json/)
      .expect(200)
      .expect((res) => expect(res.body.balances.xJEWEL).toBeDefined())
      .expect((res) => expect(res.body.balances.USDC).toBeDefined());
  });
});
...
describe('POST /evm/cancel', () => {
  it('should return 200', async () => {
    ...
    dfkchain.getWallet = jest.fn().mockReturnValue({
      address,
    });

    dfkchain.cancelTx = jest.fn().mockReturnValue({
...
describe('POST /network/poll', () => {
  it('should get a NETWORK_ERROR_CODE when the network is unavailable', async () => {
    patch(dfkchain, 'getCurrentBlockNumber', () => {
  ...
  it('should get a null in txReceipt for Tx in the mempool', async () => {
    patch(dfkchain, 'getCurrentBlockNumber', () => 1);
    patch(dfkchain, 'getTransaction', () => transactionOutOfGas);
    patch(dfkchain, 'getTransactionReceipt', () => null);
    const res = await request(gatewayApp).post('/network/poll').send({
      chain: 'dfkchain',
      network: 'mainnet',
      txHash:
  ...
  it('should get a null in txReceipt and txData for Tx that didnt reach the mempool and TxReceipt is null', async () => {
    patch(dfkchain, 'getCurrentBlockNumber', () => 1);
    patch(dfkchain, 'getTransaction', () => null);
    patch(dfkchain, 'getTransactionReceipt', () => null);
    const res = await request(gatewayApp).post('/network/poll').send({
      chain: 'dfkchain',
      network: 'mainnet',
  ...
  it('should get txStatus = 1 for a succesful query', async () => {
    patch(dfkchain, 'getCurrentBlockNumber', () => 1);
    patch(dfkchain, 'getTransaction', () => transactionSuccesful);
    patch(dfkchain, 'getTransactionReceipt', () => transactionSuccesfulReceipt);
    const res = await request(gatewayApp).post('/network/poll').send({
      chain: 'dfkchain',
      network: 'mainnet',
      txHash:
